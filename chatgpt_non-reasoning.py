...
This script sends a 14-question financial decision-making prompt to OpenAI's (this below has `chatgpt_4o` model)
...
- Set `model_name = "gpt-4.0" or "gpt_4.5"
- The same script can be used with "gpt-4.1" by simply updating the model name.
...
Output:
- A CSV file (e.g., `gpt-4o.csv`) containing columns `Trial, Q1, ..., Q14`
- One row per trial
...
log_file = os.path.join(log_folder, f"chatgpt_4o_run_{n_trials}_{timestamp}.log")
...
def parse_multi_answer_reply(reply_text, trial):
    """
    Parses lines like '1: A' or '2. 110'.
    Ignores explanations or unstructured text.
    Keeps only the first valid answer per question (1–14).
    """
    answers = {}
    lines = [line.strip() for line in reply_text.strip().splitlines() if line.strip()]
    numbered_pattern = re.compile(r"^(\d{1,2})\s*[:\.]\s*(.+)$")  # Support ':' or '.'

    for line in lines:
        match = numbered_pattern.match(line)
        if match:
            q_num = int(match.group(1))
            if 1 <= q_num <= 15 and q_num not in answers:
        ...
        if len(answers) == 15:
            break  # Stop after getting all 14
